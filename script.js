...
function setupEventListeners() {
    ...
    document.addEventListener('keydown', (e) => {
        if (e.key === 'Tab' || (e.key === 'Enter' && !state.started)) {
            e.preventDefault();
            resetTest();
...
function handleInput(e) {
    ...
    if (input.endsWith(' ')) {
        ...
        if (config.mode === 'words' && state.currentWordIndex >= state.words.length) {
            endTest();
        }
    } else {
        // Update current word
...
function startTest() {
    ...
    if (config.mode === 'time') {
        ...
        state.timerInterval = setInterval(() => {
            state.timeRemaining--;
            timerSpan.textContent = state.timeRemaining;
...
function resetTest() {
    ...
    timerSpan.textContent = config.duration;
